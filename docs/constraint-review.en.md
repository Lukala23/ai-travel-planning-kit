...
## 2026-08-24 Add a sanitized offline-mobile example (examples/egypt-trip-demo.html)

- Add `examples/egypt-trip-demo.html`: a real 13-day Egypt itinerary delivered as an offline mobile page, shown as the README showcase. Sanitization: all dates shift from 10/13–10/25 to 11/03–11/15 (weekday labels re-verified one by one; zero overlap between ranges); the origin city is replaced with Xi'an including the airport name; the traveler-relationship wording is generalized to "two travelers"; the version marker changes from "formal" to "example" with the sanitization noted in meta.note.
- Technical safeguards: the base64 image block is isolated and byte-identical (14 images intact); the tripData JSON parses (13 days / 14 places); all 13 old date values plus origin keywords return zero hits in the text regions.
- README.md and README.en.md gain a "see a real example" showcase block after the deliverables table (interactive GitHub Pages demo link + source-file link) that opens the full offline page in a browser; the examples/ comment in the repository tree is updated; the example shares the structure of the repository template (skill/*/assets/trip-mobile-template.html).
- The demo link is hosted on GitHub Pages (main branch root, at https://lukala23.github.io/ai-travel-planning-kit/examples/egypt-trip-demo.html ). Code CDNs such as jsDelivr are not used: their anti-phishing policy serves .html as text/plain, so browsers show source code instead of rendering the page; GitHub Pages returns text/html and the page is directly interactive. Pages rebuilds automatically on every push to main, with no extra publish step.
- No skill rules change; the example is not packed into the Skill ZIP or portable archives (the build script only packs skill/).
- Companion fix to the city-residue check in `scripts/validate-project.sh`: the check previously matched the removed default origin city and its airport code against raw file contents, and base64 image data embedded in the example HTML randomly contains that same airport-code trigram, producing a false positive; it now strips base64 runs of 64+ characters before matching, keeping the text-level detection unchanged while validation still passes in about 2 seconds.

## 2026-08-24 Add dining to the opening capability list

- Background: the numbered list in README.md's opening "这个项目能帮你做什么" section omitted dining research, although the dedicated `references/dining.md` module exists, the detailed capability section in the same document, the English Capabilities table, the portable city pack, and the example page's restaurant library all cover it — the one list every visitor reads first was out of sync, violating the consistency sweep rule.
- README.md: insert item 6 "研究在地饮食" after item 5 (daily route), distinguishing what locals actually eat from tourist-oriented offerings and integrating signature dishes, markets, street stalls, and priority meals into the real route; later items renumber to 7-10. The list uses Markdown auto-numbering and no hard-coded capability counts exist elsewhere to sync.
- README.en.md: add dining to the domain enumeration in section 2 (Flights, lodging, dining, driving, ...), aligning with the Dining row of the Capabilities table.
- This change only touches README copy in both editions; skill rules and deliverable structure are unchanged.

## 2026-08-24 Add destination and attraction screening to the opening list and feature section

- Background: both the opening capability list and the detailed feature section of README.md omitted destination and attraction screening, although `references/destination-research.md` is a core module, usage-guide stage C covers it, and the English Capabilities table leads with it — the Chinese README covered transport, lodging, and dining but never said what the traveler actually goes to see, a clear inconsistency under the consistency sweep rule.
- README.md opening list: insert item 4 "筛选目的地与景点" (candidate universe, marketing-noise screening, spatial structure, graded trade-offs) after item 3 (airfares); later items renumber to 5-11. The insertion point matches the workflow section's order: confirm feasibility and transport, then destination structure and lodging.
- README.md feature section: add a "目的地与景点筛选" subsection between the airfare section and the lodging/route/transport section, with four bullets (candidate scan, marketing-noise screening, spatial and temporal structure, graded trade-offs) distilled from the actual destination-research.md rules.
- README.en.md: expand the first Capabilities row from only "screens meaningful candidates" to the same decision logic as the Chinese edition (candidate universe, marketing-noise screening, grading by structure and trip purpose), aligning with the "Screen marketing noise" wording of the English destination-research.md.
- No hard-coded capability counts exist to sync (the list uses Markdown auto-numbering); this change only touches README copy and does not alter skill rules.
