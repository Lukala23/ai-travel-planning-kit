...
- README.md and README.en.md gain a "see a real example" showcase block after the deliverables table (interactive jsDelivr demo link + source-file link) that opens the full offline page in a browser; the examples/ comment in the repository tree is updated; the example shares the structure of the repository template (skill/*/assets/trip-mobile-template.html).
